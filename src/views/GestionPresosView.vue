...
<nav class="mt-3">
  ...
  <div class="container">
      <h2 class="mt-3 mb-3">Registro de presos</h2>
      <table class="table">
        <thead>
          <th>Código</th>
          <th>Nombre</th>
        ...
        <tbody>
          ...
          :key="index" >
            <td v-if="registro.tipo === 'preso'">{{ registro.codigo }}</td>
            <td v-if="registro.tipo === 'preso'">{{ registro.nombre }}</td>
            <td v-if="registro.tipo === 'preso'">{{ registro.zona }}</td>
            <td v-if="registro.tipo === 'preso'">
    ...
    <form @submit.prevent="guardarRegistro">
        ...
        <div class="mb-1 form-group">
            <label for="exampleInputCodigoA" class="form-label ">Código del preso:</label>
            <input type="text" class="form-control" pattern="[P][R][0-9]{8}" id="exampleInputCodigoA" placeholder="PR00000000" aria-describedby="codigoHelp" v-model="registro.codigo" required>
        </div>
        <div class="mb-3 form-group">
            <label for="exampleInputName" class="form-label">Nombre del preso:</label>
        ...
        <div class="form-goup mb-3">
            ...
            <select class="form-control mb-2" id="exampleInputZonaA"  v-model="registro.zona">
                <option selected value="BLOQUE A">BLOQUE A</option>
